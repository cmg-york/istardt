...
public class OrganizeTravelNewUnmarshallerTest {
    ...
    private Model model;
    private Actor actor;
    private File xmlFile = getResourceFile("xml/OrganizeTravelNew.xml");
    ...
    public void testUnmarshalHeader() {
        ...
        assertEquals("2025-05-20 13:50:46", header.getLastUpdated(), "header date");
    ...
    public void testUnmarshalActor() {
        ...
        assertEquals(8, actor.getEffects().size(), "actor getEffects size");
        ...
        assertEquals("travelOrganized", rootGoal.getName(), "root goal Name");
        assertEquals("Travel Organized", rootGoal.getAtom().getDescription(), "root goal Description");
        ...
        assertEquals("ticketsBooked", rootGoal.getChildren().get(0).getName(), "root goal child name 1");
        assertEquals("authorizationSigned", rootGoal.getChildren().get(1).getName(), "root goal child name 2");
    }
    ...
    public void testUnmarshalPredicates() {
        ...
        assertEquals("nonRefTixFailed", predicateAtom1.getTitleText(), "predicate Name");
        assertEquals("", predicateAtom1.getDescription(), "predicate Description");

        // ========= PREDICATE 2 =========
        Atom predicateAtom2 = predicates.get(1).getAtom();
        assertEquals("headGranted", predicateAtom2.getTitleText(), "predicate Name");
        assertEquals("", predicateAtom2.getDescription(), "predicate Description");

        // ========= PREDICATE 3 =========
        Atom predicateAtom3 = predicates.get(2).getAtom();
        assertEquals("flightsExist", predicateAtom3.getTitleText(), "predicate Name");
        assertEquals("", predicateAtom3.getDescription(), "predicate Description");
    }
    ...
    public void testUnmarshalVariables() {
        ...
        assertEquals("cost", variableAtom1.getTitleText(), "variable Name");
        assertEquals("", variableAtom1.getDescription(), "variable Description");
    }
    ...
    public void testUnmarshalCrossRuns() {
        ...
        assertTrue(crossRuns.get(0) instanceof Quality,
                "Expected the element to be a Quality but was " + crossRuns.get(0).getClass().getName());
        assertEquals("avoidMoneyLoss", crossRunAtom1.getTitleText(), "crossRun element Name");
        assertEquals("Avoid Money Loss", crossRunAtom1.getDescription(), "crossRun element  Description");

        // ========= CROSSRUN 2 =========
        Atom crossRunAtom2 = crossRuns.get(1).getAtom();
        assertTrue(crossRuns.get(1) instanceof Quality,
                "Expected the element to be a Quality but was " + crossRuns.get(1).getClass().getName());
...
//        assertEquals("", crossRunAtom3.getDescription(), "crossRun element Description");
        ...
        assertTrue(crossRuns.get(3) instanceof Variable,
                "Expected the element to be a Variable but was " + crossRuns.get(3).getClass().getName());
        assertEquals("cost", crossRunAtom4.getTitleText(), "crossRun element Name");
        assertEquals("", crossRunAtom4.getDescription(), "crossRun element Description");
    }
//
//    @Test
